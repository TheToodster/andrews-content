...
function ENT:Use( ply )
    ...
    if IsValid( ply ) && ply:IsPlayer() then
		...
		local getitem = table.Random( Loot.RareDarkInner )
		local advdirandxp = table.Random( Loot.RareDarkInnerXP )
		if howrare >= 1 && howrare <= 150 then
        	wOS:HandleItemPickup( ply, getitem )
			ply:AddSkillXP( advdirandxp )
			ply:PrintMessage( HUD_PRINTCENTER, "You found a " .. getitem .. " and gained " .. advdirandxp .. "xp from this lootbox." )
		end
		
		local getitem2 = table.Random( Loot.CommonDarkInner )
		local dirandxp = table.Random( Loot.CommonDarkInnerXP )
		if howrare >= 151 && howrare <= 1000 then
			wOS:HandleItemPickup( ply, getitem2 )
			ply:AddSkillXP( dirandxp )
			ply:PrintMessage( HUD_PRINTCENTER, "You found a " .. getitem2 .. " and gained " .. dirandxp .. "xp from this lootbox." )
		end
