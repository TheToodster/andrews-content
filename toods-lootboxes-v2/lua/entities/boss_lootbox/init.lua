...
if Loot.BossLootActive then
    hook.Add( "OnNPCKilled", "LootBoss", function( npc, killer ) 
        if IsValid( killer ) && killer:IsPlayer() then
            if Loot.BossNPCs[npc:GetClass()] then
            local LootSpawn = ents.Create( "boss_lootbox" )
                LootSpawn:SetPos( npc:LocalToWorld( Vector( 0, 80, 10 ) ) )
				LootSpawn:Spawn()
				LootSpawn:Activate()
            end
        end
...
function ENT:Use( ply )
    ...
    if IsValid( ply ) && ply:IsPlayer() then
		...
		local getitem = table.Random( Loot.RareBossItems )
		local dirandxp = table.Random( Loot.RareBossXP )
		...
		local getitem2 = table.Random( Loot.CommonBossItems )
		local randxp = table.Random( Loot.CommonBossXP )
