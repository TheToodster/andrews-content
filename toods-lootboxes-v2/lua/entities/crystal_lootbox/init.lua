...
function ENT:Use( ply )
    ...
    if IsValid( ply ) && ply:IsPlayer() then
		...
		local getitem = table.Random( Loot.RareCrystals )
		local advrandxp = table.Random( Loot.RareCrystalXP )
		if howrare >= 1 && howrare <= 150 then
			wOS:HandleItemPickup( ply, getitem )
			ply:AddSkillXP( advrandxp )
			ply:PrintMessage( HUD_PRINTCENTER, "You found a " .. getitem .. " and gained " .. advrandxp .. "xp from this lootbox." )
		end
		
		local getitem2 = table.Random( Loot.CommonCrystals )
		local randxp = table.Random( Loot.CommonCrystalXP )
